...
async function handlePermissionCommand(ctx, chatId, arg) {
    ...
    const executeHost = async (line) => {
        try {
            ...
            if (live === undefined)
                return null;
            const result = await commands.execute(live.agent, line, new AbortController().signal);
            if (result === undefined) {
    ...
    const relayMarked = async (result) => {
    ...
    if (trimmed === '') {
        ...
        if (bare !== null)
            await ctx.sendToChat(chatId, bare.text + '\n' + PERMISSION_USAGE_HINT);
        return;
    }
    if (/^\d+$/.test(trimmed)) {
