...
function persistEntry(sessionId, interimOverride, goal) {
    if (interimOverride === undefined && goal === undefined)
        return sessionId;
    ...
    async saveMapping() {
        try {
            ...
            for (const [chatId, evicted] of this.evictedChats)
                mapping[chatId] = persistEntry(evicted.session, evicted.interimOverride, evicted.goal);
