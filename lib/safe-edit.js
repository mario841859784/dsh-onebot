...
export function checkPathAllowed(root, filepath) {
    if (root === '')
        return 'safeEditRoot 未配置：code_safe_edit 已禁用（请先在配置中指定可编辑根目录）。';
    const r = resolve(root);
    const p = resolve(filepath);
    if (p === r)
        return '目标不能是根目录本身。';
    if (!p.startsWith(r + sep) && p !== r)
        return `路径超出可编辑根目录 ${r}。`;
    return null;
}
/** All non-overlapping match start indices of `old` in `content`. */
function collectPositions(content, old) {
    const positions = [];
    let pos = 0;
    for (;;) {
...
export function alignWhitespace(content, old, newText) {
    const oldLines = old.split('\n');
    const contentLines = content.split('\n');
    const oldStripped = oldLines.map(line => line.trim());
    if (oldStripped.length === 0 || oldStripped[0] === '')
        return null;
    for (let i = 0; i + oldLines.length <= contentLines.length; i += 1) {
        ...
        const alignedNew = newLines.map((line, index) => {
            if (line.trim() === '')
                return line;
            if (index === 0)
                return firstIndent + line.slice(oldIndent.length);
...
export async function syntaxCheckFile(filepath, runCheck = defaultSyntaxCheck) {
...
async function defaultSyntaxCheck(file) {
    return await new Promise((resolvePromise, reject) => {
        execFile('node', ['--check', file], { timeout: 15_000 }, (error, stdout, stderr) => {
            if (error === null)
                resolvePromise('');
            else
                resolvePromise(String(stderr ?? stdout).trim());
        });
    });
}
...
async function backupFor(backupDir, filepath) {
...
async function pruneBackups(backupDir, filepath, keep) {
    try {
        const stem = filepath.split(sep).pop() ?? 'file';
        const prefix = stem + '.';
 ...
 */
export async function safeEdit(request, options) {
    ...
    const common = { ok: false, file: filepath };
    const pathError = checkPathAllowed(options.root, filepath);
    if (pathError !== null)
        return { ...common, ok: false, error: pathError };
    const stats = await stat(filepath).catch(() => undefined);
    if (stats === undefined)
        return { ...common, ok: false, error: '文件不存在：' + request.filepath };
    if (!stats.isFile())
        return { ...common, ok: false, error: '路径不是文件：' + request.filepath };
    if (stats.size > SAFE_EDIT_MAX_SIZE)
        return { ...common, ok: false, error: '文件超过 5MB 上限，拒绝编辑。' };
    if (mode === 'replace' && old === '')
        return { ...common, ok: false, error: 'old 参数不能为空字符串（空替换会损毁文件）。' };
    if (mode === 'insert_at_line' && newText === '')
        return { ...common, ok: false, error: 'insert_at_line 模式 new 不能为空。' };
    if (occurrence < 0)
        return { ...common, ok: false, error: 'occurrence 不能为负数。' };
    if (occurrence > 0 && replaceAll)
        return { ...common, ok: false, error: 'occurrence 与 replace_all 不能同时使用。' };
    ...
    if (mode === 'replace') {
        ...
        if (occurrence > oldCount)
            return { ...common, ok: false, error: `occurrence=${occurrence} 超过匹配总数 ${oldCount}。` };
        // ── backup then edit ──
        let backupPath;
        try {
        ...
        if (!(await writeGuarded(filepath, next, common)))
            return common;
...
function normalizeReadError(path) {
    return (error) => {
...
async function writeGuarded(filepath, next, common) {
    try {
...
async function runSyntaxCheck(filepath, custom) {
    if (custom !== undefined)
        return await custom(filepath);
...
async function finishEdit(common, filepath, options) {
...
export async function safeRollback(filepath, backupDir, root, backupName) {
    ...
    if (backupName === undefined || backupName === '') {
        ...
        if (names.length === 0)
            return { ok: false, error: '没有找到该文件的备份。' };
        backupPath = backupDir + sep + names[0];
    }
    if (!(await stat(backupPath).catch(() => undefined)))
        return { ok: false, error: '备份不存在：' + backupName };
...
export async function listBackups(backupDir, filepath) {
    await mkdir(backupDir, { recursive: true });
    ...
    const backups = items.filter((item) => item !== null).sort((a, b) => b.name.localeCompare(a.name));
