...
export class ChatBridge {
    ...
    start() {
        ...
        this.sessionEventOff = ctx.on('session/event', (session, event) => {
            this.onSessionEvent(session, event);
        });
        this.sessionFlushOff = ctx.on('session/flush', (session) => {
    ...
    renderQuestionCard(rawArguments) {
        if (typeof rawArguments !== 'string' || rawArguments === '')
            return undefined;
        try {
            const parsed = JSON.parse(rawArguments);
