...
export class FontWidthSource {
    font;
    constructor(font) {
        this.font = font;
    }
    charWidth(ch, size, code) {
        if (ch === '')
            return 0;
...
function linesFor(content, ctx, budget, size) {
 ...
 */
export function renderInlineParts(card, x, y, parts, size, fill, ctx) {
    let cx = x;
...
export function elementHeight(el, ctx) {
    switch (el.kind) {
        case 'text': {
            if (el.content.trim() === '')
                return 10;
...
export function headerFontSize(level) {
    return 42 - (level - 1) * 4;
}
/** Wrap code-block lines at the code budget. */
function wrapCodeLines(content, ctx) {
    const wrapped = [];
...
export function renderElement(el, card, ctx, x, y) {
    switch (el.kind) {
        case 'text': {
            if (el.content.trim() === '')
                return y + 10;
        ...
        case 'list': {
            ...
            if (family !== undefined)
                card.drawRun('•', x + 5, y, family, ctx.size, COLORS.ink);
            let ty = y;
            for (const line of lines) {
        ...
        case 'ordered': {
            ...
            const family = card.font.familyFor('cjk');
            if (family !== undefined)
                card.drawRun(String(el.number) + '.', x + 5, y, family, ctx.size, COLORS.ink);
            let ty = y;
            for (const line of lines) {
            ...
            for (const rowLines of cellLines) {
                for (const line of rowLines[ci] ?? []) {
                    ...
                    for (const part of line)
                        w += segWidth(part, ctx.widthSource, size);
                    if (w > maxW)
                        maxW = w;
                }
            }
...
function renderTable(el, layout, card, ctx, x, y) {
    void el;
    const size = ctx.size;
    const tableX = x + 10;
