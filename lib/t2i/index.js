...
import { FontWidthSource, elementHeight, renderElement } from './elements.js';
...
function drawFooter(card, brand, y) {
...
function fontManagerFor(options) {
    const key = JSON.stringify([options.fontFiles ?? [], options.fontFamilies ?? []]);
    if (cachedManager !== undefined && cachedManager.key === key)
 ...
 * @returns PNG bytes.
 */
export function renderTextImage(text, options = {}) {
    ...
    if (options.title !== undefined && options.title !== '') {
        ...
        if (family !== undefined) {
            ...
            card.drawBodyRuns(options.title, 24, center - (ascent + descent) / 2, 52, COLORS.white);
        }
        y = 10 + TOPBAR_H;
    }
    for (const el of blocks) {
