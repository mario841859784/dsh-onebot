...
export function codeFontSize(fullSize) {
...
export class CardCanvas {
    ...
    strokeRoundRect(x, y, w, h, radius, color) {
     ...
     */
    drawRun(text, x, y, family, size, fill, dy = 0) {
    ...
    runWidth(text, family, size) {
        let total = 0;
        for (const ch of text)
            total += this.font.charWidth(ch, family, size);
        return total;
    }
    /**
     ...
     */
    drawBodyRuns(text, x, y, size, fill, skipEmoji = false) {
        ...
        while (i < chars.length) {
            ...
            if (cls === 'emoji') {
                ...
                if (emojiFamily !== undefined) {
                    ...
                    if (!skipEmoji && width > 0)
                        this.drawRun(chars[i], cx, y, emojiFamily, size, fill);
                    cx += width;
                }
                else {
                    ...
                    if (family !== undefined) {
                        if (!skipEmoji)
                            this.drawRun(chars[i], cx, y, family, size, fill);
     ...
     * @returns the advanced x.
     */
    drawCodeRuns(text, x, y, size, fill) {
        ...
        while (i < chars.length) {
            ...
            while (j < chars.length) {
                ...
                if (next === 'zwj')
                    break;
                if (next === 'emoji' && cls !== 'emoji')
                    break;
                if (next !== 'emoji' && cls === 'emoji')
                    break;
                if ((next === 'ascii') !== (cls === 'ascii'))
                    break;
                j += 1;
            }
            const run = chars.slice(i, j).join('');
