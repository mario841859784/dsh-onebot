...
function isEmojiCp(cp) {
    ...
    if (cp >= 0x2B00 && cp <= 0x2BFF)
        return true;
    if (cp === 0x00A9 || cp === 0x00AE || cp === 0x2122)
        return true;
    return false;
}
/**
...
export class FontManager {
    ...
    codeFamilyFor(cls) {
        if (cls === 'emoji')
            return this.emojiFamily;
        if (cls === 'ascii')
            return this.monoFamily ?? this.cjkFamilies[0];
        return this.cjkFamilies[0];
    }
    /**
    ...
    init(config) {
        ...
        if (cjkFirst !== undefined)
            this.cjkFamilies.push(cjkFirst);
