 ...
 */
export function registerTools(ctx, bridge, connection, limits, editing) {
    const disposers = [];
    const register = (tool) => {
    ...
    if (editing !== undefined && editing.safeEditRoot !== '') {
        const renderEdit = (_args, value) => {
            const v = value;
            if (v.ok) {
                return [{ type: 'text', text: '✅ 已修改：' + (v.file ?? '') + (v.whitespaceAligned === true ? '（空白已对齐匹配）' : '') + (v.lineNumbersStripped === true ? '（已剥行号前缀）' : '') }];
            }
            const detail = (v.error ?? '') + (v.proposal !== undefined ? '\n' + String(v.proposal) : '');
            return [{ type: 'text', text: '❌ 未修改：' + detail }];
        };
        const assertEditingAllowed = (sessionId) => {
        ...
        register(defineTool({
            ...
            execute: async (args, exec) => {
                ...
                if (typeof a.filepath !== 'string' || a.filepath === '')
                    throw new Error('code_safe_edit: filepath 必填');
        ...
        register(defineTool({
            ...
            output: { schema: { type: 'json' }, render: (_a, value) => {
                    const v = value;
                    return [{ type: 'text', text: v.ok === true ? '✅ 已回滚：' + (v.file ?? '') : '❌ 回滚失败：' + (v.error ?? '') }];
                } },
            isConcurrencySafe: () => true,
            execute: async (args, exec) => {
                const a = args;
                if (typeof a.filepath !== 'string' || a.filepath === '')
                    throw new Error('code_safe_rollback: filepath 必填');
                assertEditingAllowed(exec.agent?.session?.id);
                return await safeRollback(a.filepath, editing.backupDir !== '' ? editing.backupDir : editing.safeEditRoot + '/.backups', editing.safeEditRoot, typeof a.backup_name === 'string' && a.backup_name !== '' ? a.backup_name : undefined);
            },
        }));
        register(defineTool({
            ...
            output: { schema: { type: 'json' }, render: (_a, value) => {
                    const v = value;
                    return [{ type: 'text', text: (v.total ?? 0) + ' 个备份：' + (v.backups ?? []).map(b => b.name).join(', ') }];
                } },
            isConcurrencySafe: () => true,
            execute: async (args, exec) => {
                const a = args;
                assertEditingAllowed(exec.agent?.session?.id);
                return await listBackups(editing.backupDir !== '' ? editing.backupDir : editing.safeEditRoot + '/.backups', typeof a.filepath === 'string' && a.filepath !== '' ? a.filepath : undefined);
            },
        }));
    }
    return () => {
