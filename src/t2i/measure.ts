...
export function textWidth(source: WidthSource, text: string, size: number): number {
  let total = 0
  for (const ch of text) total += source.charWidth(ch, size, false)
  return total
}

/** Summed width of a code content run (mono/small-CJK widths). */
export function codeWidth(source: WidthSource, text: string, size: number): number {
  let total = 0
  for (const ch of text) total += source.charWidth(ch, size, true)
  return total
}
 ...
 */
export function segWidth(part: InlinePart, source: WidthSource, size: number): number {
  if (part.cls === 'code') return codeWidth(source, part.content, size) + 14
  if (part.cls === 'bold') return textWidth(source, part.content, size) + 1
  if (part.cls === 'italic') return textWidth(source, part.content, size) + 8
 ...
 * @returns lines.
 */
export function splitToFit(text: string, maxWidth: number, width: (ch: string) => number): string[] {
  if (text === '') return []
  const lines: string[] = []
  let cur = ''
  let curW = 0
...
function textWidthOf(text: string, width: (ch: string) => number): number {
  let total = 0
 ...
 */
export function splitInlineLines(parts: InlinePart[], maxWidth: number, source: WidthSource, size: number): InlinePart[][] {
  ...
  for (const part of parts) {
    ...
    const widthFn = (ch: string): number => source.charWidth(ch, size, part.cls === 'code')
    const segments = splitToFit(content, maxWidth - reserve, widthFn)
    for (const seg of segments) {
      const segPart: InlinePart = { cls: part.cls, content: seg }
      ...
      if (lines[lines.length - 1].length > 0 && curW + w > maxWidth) {
        ...
        if (part.cls === null && seg.length > 0 && HEAD_BANNED.has(seg[0])) {
          let i = 0
          while (i < seg.length && HEAD_BANNED.has(seg[i])) i += 1
